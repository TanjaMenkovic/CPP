...
void ScalarConverter::printFloat() const 
{
    ...
    else 
    {
        try 
        {
            // Check if the value is within float limits
            if (_value < std::numeric_limits<float>::lowest() || 
...
void ScalarConverter::printDouble() const 
{
    ...
    try 
    {
        // Convert and validate double representation
        double valueNew = std::stod(_input);

        if (valueNew != _value) // Check if conversion is valid
        {
            throw std::overflow_error("Value not representable as double");
        }
...
bool ScalarConverter::incorrectChar()
{
    ...
    for(size_t i = 0; i < _input.length(); i++)
    {
        if (_input[i] == '.') numOfDot++;
        if (!isValidCharacter(_input[i]) || numOfDot > 1 || 
            (_input[i] == 'f' && (i != _input.length() - 1)) || // 'f' must be at the end
            ((_input[i] == '+' || _input[i] == '-') && i != 0))
        {
            std::cout << "char: impossible\nint: impossible\nfloat: impossible\ndouble: impossible\n";
            return false;
